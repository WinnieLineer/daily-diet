  ...
  return (
    <div className="relative">
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            <div className="fixed inset-0 z-[55] sm:hidden bg-black/20 backdrop-blur-sm" onClick={() => setIsOpen(false)} />
            <NeoCard className="fixed top-20 left-4 right-4 sm:absolute sm:top-14 sm:right-0 sm:left-auto z-[60] w-auto sm:w-72 space-y-4 animate-in fade-in slide-in-from-top-2">
              ...
              <div className="space-y-3">
                <div>
                  <label className="text-xs font-black uppercase tracking-wider block mb-1 text-gray-500">每日熱量 (kcal)</label>
                  <input 
                    type="number"
                    value={goals.calories}
                    onChange={(e) => setGoals({ ...goals, calories: e.target.value })}
                    className="w-full border-4 border-black p-2 rounded-xl font-mono focus:outline-none focus:ring-4 ring-accent/20 transition-all font-bold"
                  />
                </div>
                <div>
                  <label className="text-xs font-black uppercase tracking-wider block mb-1 text-gray-500">每日蛋白質 (g)</label>
                  <input 
                    type="number"
                    value={goals.protein}
                ...
                <div>
                  <label className="text-xs font-black uppercase tracking-wider block mb-1 text-gray-500">每日飲水 (ml)</label>
                  <input 
                    type="number"
                    value={goals.water}

...
function doGet(e) {
  try {
    ...
    if (action === 'diagnoseDailySummary') {
      ...
      const userGistId = props.getProperty(`USER_GIST_${targetUser}`) || '';
      const liffId = props.getProperty('LINE_LIFF_ID') || props.getProperty('LIFF_ID') || '2011098313-nFOisgmf';
      let flexJson = '';
      let flexError = null;
      let summaryFlex = null;
      let lineValidation = null;
      let pushResult = null;

      try {
        summaryFlex = generateDailySummaryFlex(targetUser, null, liffId, userGistId, props);
        if (summaryFlex && targetUser) {
          attachQuickReply(summaryFlex, targetUser, props);
        }
      ...
      if (summaryFlex && CHANNEL_ACCESS_TOKEN) {
        try {
          const vRes = UrlFetchApp.fetch("https://api.line.me/v2/bot/message/validate/reply", {
            method: "post",
            headers: {
            ...
            payload: JSON.stringify({
              messages: [summaryFlex]
            }),
            muteHttpExceptions: true
          });
          lineValidation = {
            status: vRes.getResponseCode(),
            body: vRes.getContentText()
          };
        } catch (vErr) {
          lineValidation = { error: vErr.stack || vErr.message };
        }

        if (e?.parameter?.push === 'true' && targetUser) {
          try {
            const pRes = UrlFetchApp.fetch("https://api.line.me/v2/bot/message/push", {
              method: "post",
              headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${CHANNEL_ACCESS_TOKEN}`
            ...
            pushResult = {
              status: pRes.getResponseCode(),
              body: pRes.getContentText()
            };
          } catch (pErr) {
            pushResult = { error: pErr.stack || pErr.message };
          }
        }
      }

      const logs = (typeof getRecentLogsData === 'function') ? getRecentLogsData(30, 1) : [];
      return ContentService.createTextOutput(JSON.stringify({
        ...
        lineValidation,
        pushResult,
        recentLogs: logs.map(l => ({ time: l.time, type: l.type, user: l.userName, userId: l.userId, input: l.input, output: l.output }))
